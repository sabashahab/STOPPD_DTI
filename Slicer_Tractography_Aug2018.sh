...
# Resample Mask using QCed.nrrd as the reference
if [ ! -e $outputfolder/${stem}_MASK_oriented.nrrd ]; then
  ...
  echo "ResampleScalarVectorDWIVolume(MASK) was already run on this subject!"
fi

# Resample Scalar using QCed.nrrd as the reference
if [ ! -e $outputfolder/${stem}_SCALAR_oriented.nrrd ]; then
  ResampleScalarVectorDWIVolume -R ${outputfolder}/${stem}_QCed.nrrd ${outputfolder}/${stem}_SCALAR.nrrd ${outputfolder}/${stem}_SCALAR_oriented.nrrd
else
  echo "ResampleScalarVectorDWIVolume(SCALAR) was already run on this subject!"
fi
...
if [ ! -e $outputfolder/${stem}_DTI.nrrd ]; then
  DWIToDTIEstimation \
    --enumeration WLS \
    --shiftNeg \
    -m $outputfolder/${stem}_MASK_oriented.nrrd \
    $outputfolder/${stem}_QCed.nrrd \
    $outputfolder/${stem}_DTI.nrrd \
    $outputfolder/${stem}_SCALAR_oriented.nrrd
else 
